...
def request_stream(data_stream, rate, single_utterance=True, interim_results=True):
...
def listen_print_loop(recognize_stream):
    ...
    for resp in recognize_stream:
        ...
        if not result.is_final:
            ...
            if re.search(r'\b(por favor)\b', transcript, re.I):
                return(transcript)
...
def start():
    with record_audio(RATE, CHUNK) as buffered_audio_data:
        ...
        try:
            result = listen_print_loop(recognize_stream)
            ...
            if "deadline too short" in e.message:
                return None
            else:
                raise
...
def initialize(language="en-US", timeout=10, credentials_path="resources/google-cloud-speech.json"):
    global LANGUAGE, SERVICE, DEADLINE_SECS
